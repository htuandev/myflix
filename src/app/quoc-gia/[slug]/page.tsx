...
export async function generateMetadata({ params, searchParams }: Params): Promise<Metadata> {
  const { name } = await fetchCategory(params.slug, 'countries');
  return {
...
export default async function Page({ searchParams, params }: Params) {
  ...
  const { movies, totalPages } = await fetchMovies(searchParams.page, { countries: extractIdFromSlug(params.slug) });
