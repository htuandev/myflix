...
export async function generateMetadata({ searchParams }: { searchParams: NextQuery }): Promise<Metadata> {
  const { movies } = await fetchMovies(searchParams.page, { type: ContentType.Vietnamese });

  const title = `Top Phim Thuyết Minh Hay Mới Nhất ${CURRENT_YEAR}${Number(searchParams.page) > 1 ? ` - Trang ${searchParams.page}` : ''} | Myflix`;
  const description = `Danh sách phim thuyết minh mới cập nhật | Myflix`;
