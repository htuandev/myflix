...
export async function generateMetadata({ searchParams }: { searchParams: NextQuery }): Promise<Metadata> {
  return {
    title: `Top Phim Hoạt Hinh Hay Mới Nhất ${CURRENT_YEAR}${Number(searchParams.page) > 1 ? ` - Trang ${searchParams.page}` : ''} | Myflix`
  };
}
...
export default async function Page({ searchParams }: { searchParams: NextQuery }) {
  ...
  return (
    <section className=' container flex flex-col'>
      <h1 className='text-heading mb-6 text-center'>Phim Hoạt Hinh Mới Cập Nhật</h1>
