...
export async function generateMetadata({ searchParams }: { searchParams: NextQuery }): Promise<Metadata> {
  const { movies } = await fetchMovies(searchParams.page, { type: ContentType.Series });

  const title = `Top Phim Bộ Hay Mới Nhất ${CURRENT_YEAR}${Number(searchParams.page) > 1 ? ` - Trang ${searchParams.page}` : ''} | Myflix`;
  const description = `Danh sách phim bộ mới cập nhật | Myflix`;
